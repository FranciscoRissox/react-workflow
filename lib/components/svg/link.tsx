...
const Link = ({ coordinates,style }: LinkProps) => {
  ...
  return (
    ...
    <marker id={id+"arrowReverse"} markerWidth={markerWidth} markerHeight={markerWidth} refX="0" refY="3" orient="auto">
    ...
    <path
      ...
      markerEnd={style?.endFigure === LineFigures.ARROW ? 'url(#'+id+'arrow)' : style?.endFigure === LineFigures.CIRCLE ? 'url(#'+id+'circle)' : "none"}
      markerStart={style?.startFigure === LineFigures.ARROW ? 'url(#'+id+'arrowReverse)' : style?.startFigure === LineFigures.CIRCLE ? 'url(#'+id+'circle)' : "none"}
    />
    </>
  );
};

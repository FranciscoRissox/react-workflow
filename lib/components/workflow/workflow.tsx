...
export const WorkFlow = ({
  state: { nodes, links },
  actions: { addLink, setNode, setNodeRef },
  height = '500px',
  width = '500px',
  ...
  return (
    ...
    >
      ...
      <div
        className={styles.workflowcontent + ' ' + styles.workflow + ' ' + styles[backgroundColor]}
        ref={ref}
        style={{
          height: '25000px',
          width: '25000px',
        ...
        {nodes.map((node, idx) => {
          return (
            <WorkflowNode
              ...
              links={filterLinks(node.id, links)}
              selectNode={selectNode}
              canvasRef={ref as any}
              key={node.id}
              nodeData={node}
              setPosition={pos => setNode(idx, { ...node, position: pos })}
              canvasRect={boundRect}
              scale={scale}
            />
          );
        })}

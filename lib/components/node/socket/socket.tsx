...
export const Socket = React.forwardRef<HTMLDivElement,{selectNode: (position:SocketPosition) => boolean;position: SocketPosition;scale: number;connected?: boolean}>((props,ref) => {
    const [isConnecting,setIsConnecting] = useState(false)

    useEffect(()=>{if(props.connected) setIsConnecting(false)},[props.connected])
    ...
    return (
        <>
            <div ref={ref} className={`${styles.socket} ${isConnecting ? styles.connecting : ""} ${props.connected ? styles.connected : ""}`} onClick={!props.connected ? onClick : undefined} style={{position:"absolute",transformOrigin: "center",transform: `scale(${props.scale}) translate(-50%,-50%)`,top:positionY(),left:positionX()}}></div>
        </>
